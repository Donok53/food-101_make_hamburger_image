...
def main():
    ...
    parser.add_argument('--gpu', type=int, default=1, help='쓸 GPU 번호 (예: 1 => cuda:1)')
    ...
    parser.add_argument('--lambda_patch', type=float, default=1.0, help='Patch 손실 가중치')
    ...
    # 모델, 옵티마이저 초기화
    G = Generator(512,0,512, img_resolution=256, img_channels=3).to(device)
    D_global = Discriminator(0,256,3).to(device)
    D_patch  = Discriminator(0,256,3).to(device)
